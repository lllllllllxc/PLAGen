...
import pickle
from .utils_x import my_con_loss
...
class image_PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout, max_len=5000):
        super(image_PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)

        pe = torch.zeros(max_len, d_model).to("cuda:0")
...
class EncoderDecoder(AttModel):
    ...
    '''这个方法主要是对输入的特征进行处理和准备，以便后续在模型中使用。'''
    ...
    '''这个方法主要用于对输入的注意力特征att_feats进行一系列处理，包括裁剪、嵌入、生成查询矩阵、进行特征交互等操作，
    为模型的后续计算准备好特征和相关的掩码、矩阵等数据。'''
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None, labels=None,):
        # 添加位置向量
        positional_encoding = image_PositionalEncoding(2048, self.dropout)
        att_feats = att_feats + positional_encoding(att_feats)
    ...
    '''这个方法是模型的核心计算部分，主要用于处理输入的单词索引it以及各种特征和状态信息，通过调用模型的解码部分进行计算，最终返回当前时间步的输出和更新后的状态。'''
