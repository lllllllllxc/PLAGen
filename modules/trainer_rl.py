...
from XproNetRL.modules.optimizers import set_lr
from XproNetRL.modules.rewards import get_self_critical_reward, init_scorer
from XproNetRL.modules.loss import compute_loss
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch_IU_Xray(self, epoch):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks, labels) in enumerate(
            ...
            self.model.eval()
            with torch.no_grad():
                greedy_res, _ = self.model(images, labels=labels, mode='sample',
                                           update_opts={'sample_method': self.args.sc_sample_method,
                                                        'beam_size': self.args.sc_beam_size})

            self.model.train()
            gen_result, sample_logprobs = self.model(images, labels=labels, mode='sample',
            ...
            labels = labels.to(self.device)
            output, _, _, _, _ = self.model(images, reports_ids, labels=labels, mode='train')
