...
from XproNet import modules as utils
from XproNet.modules.caption_model import CaptionModel
...
'''要用于图像字幕生成任务中的注意力模型。它通过处理输入的特征、使用注意力机制和循环神经网络来生成图像的描述字幕，并提供了不同的采样方法和束搜索策略。'''
...
class AttModel(CaptionModel):
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, labels = None, update_opts={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, labels, query_matrix, cmn_masks = self._prepare_feature(fc_feats, att_feats, att_masks, labels=labels)
        ...
        for t in range(self.max_seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,query_matrix,cmn_masks, labels, state,
